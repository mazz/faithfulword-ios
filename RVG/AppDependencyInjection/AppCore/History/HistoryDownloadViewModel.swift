//
//  HistoryDownloadViewModel.swift
//  FaithfulWord
//
...
internal final class HistoryDownloadViewModel: HistoryMediaViewModeling {
    ...
    //                self?.assetPlaybackService.playables.value = playables
        ...
        func initialFetch() {
            os_log("HistoryDownloadViewModel initialFetch", log: OSLog.data)
            
            downloadService.fetchStoredFileDownloads()
                .subscribe(onSuccess: { downloads in
                    os_log("downloads: %{public}@", log: OSLog.data, String(describing: downloads))
                })
                .disposed(by: bag)
